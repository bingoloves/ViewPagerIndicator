...
public class ShadowTransformer implements ViewPager.OnPageChangeListener, ViewPager.PageTransformer {
    ...
    private IShadowAdapter mAdapter;
    private float mLastOffset;
    private boolean mScalingEnabled;

    public ShadowTransformer(ViewPager viewPager, IShadowAdapter adapter) {
        ...
        if (currentCard != null) {
            ...
            if (currentCard instanceof CardView){
                ((CardView)currentCard).setCardElevation((baseElevation + baseElevation
                        * (IShadowAdapter.MAX_ELEVATION_FACTOR - 1) * (1 - realOffset)));
            }
        }
        ...
        if (nextCard != null) {
            ...
            if (nextCard instanceof CardView){
                ((CardView)nextCard).setCardElevation((baseElevation + baseElevation
                        * (IShadowAdapter.MAX_ELEVATION_FACTOR - 1) * (realOffset)));
            }
        }
